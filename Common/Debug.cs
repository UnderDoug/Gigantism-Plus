...
namespace HNPS_GigantismPlus
{
    ...
    public static class Debug
    {
        ...
        {
            Entry(Verbosity, Text, Indent + 1, Toggle: Toggle);
            Divider(Verbosity, HONLY, 37, Starter: SPACE + BANDR, Finisher: TANDL, Indent, Toggle: Toggle); // <
        }
            ...
            where E : MinEvent, new()
        {
            ...
            {
                Entry(2, $"Registered", $"{typeof(H).Name} ({typeof(E).Name}.OperatorID: {e.ID})", Indent: 2, Toggle: Toggle);
                flag = true;
            }
            else if (Game != null)
            {
                Entry(2, $"Failed to register {typeof(H).Name} ({typeof(E).Name}.OperatorID: {e.ID})", Indent: 2, Toggle: Toggle);
            }
            else
            {
                Entry(2, $"The.Game null, couldn't register {typeof(H).Name} ({typeof(E).Name}.OperatorID: {e.ID})", Indent: 2, Toggle: Toggle);
            }
            return flag;
        }
