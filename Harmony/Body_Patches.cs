    ...
    public static class Body_Patches
    {
        ...
        public static bool FireEventOnBodyparts_SendNaturalEquipmentEvent_Prefix(ref Body __instance, Event E, ref Event ___eBodypartsUpdated)
        {
            ...
            Debug.Entry(4, 
                ...
                + $"{nameof(FireEventOnBodyparts_SendNaturalEquipmentEvent_Prefix)}"
        ...
        public static void RegenerateNonDefaultNaturalEquipment(GameObject Creature)
        {
            ...
            if (!Creature.Is(null) && Creature.HasPart<Inventory>())
            {
                ...
                if (Creature.TryGetGameObjectBlueprint(out GameObjectBlueprint Blueprint) && Blueprint.Inventory != null)
                {
                    ...
                    {
                        Debug.Divider(4, HONLY, Count: 25, Indent: indent, Toggle: doDebug);
                        Debug.LoopItem(4, $"inventoryObject.Blueprint", $"{inventoryObject.Blueprint}", Indent: indent, Toggle: doDebug);
                        if (TryGetGameObjectBlueprint(inventoryObject.Blueprint, out GameObjectBlueprint itemBlueprint))
                        {
                            if (itemBlueprint.IsNatural())
                            {
                                Debug.CheckYeh(4, $"Item is Natural", Indent: indent, Toggle: doDebug);
                            ...
                            {
                                Debug.CheckNah(4, $"Item is not Natural", Indent: indent, Toggle: doDebug);
                            }
                        }
                    }
                    Debug.Divider(4, HONLY, Count: 25, Indent: --indent, Toggle: doDebug);
                    Debug.Entry(4, $"x foreach (InventoryObject inventoryObject in Blueprint.Inventory) >//", Indent: indent, Toggle: doDebug);

                    Debug.Entry(4, $"Getting list of currentItemBlueprints that are Natural", Indent: indent, Toggle: doDebug);
                    Dictionary<string, int> currentItemBlueprints = new();
                    Debug.Entry(4, $"> foreach (GameObject item in ParentObject.GetEquippedObjects())", Indent: indent, Toggle: doDebug);
                    foreach (GameObject item in Creature.GetEquippedObjects())
                    {
                        Debug.Divider(4, HONLY, Count: 25, Indent: indent + 1, Toggle: doDebug);
                        Debug.LoopItem(4, $"item.Blueprint", $"{item.Blueprint}", Indent: indent + 1, Toggle: doDebug);
                        if (item.IsNatural())
                        {
                            Debug.CheckYeh(4, $"Item is Natural", Indent: indent + 2, Toggle: doDebug);
                            ...
                            Debug.Entry(4, $"{item.Blueprint}, {currentItemBlueprints[item.Blueprint]}", Indent: indent + 3, Toggle: doDebug);
                        }
                        else
                        {
                            Debug.CheckNah(4, $"Item is not Natural", Indent: indent + 1, Toggle: doDebug);
                        }
                    }
                    Debug.Divider(4, HONLY, Count: 25, Indent: indent, Toggle: doDebug);
                    ...
                    Debug.Entry(4, $"Reducing blueprintItemBlueprint counts by number of equivalent currentItemBlueprints", 
                        Indent: indent, Toggle: doDebug);
                    Debug.Entry(4, $"> foreach ((string blueprint, int number) in currentItemBlueprints)", Indent: indent, Toggle: doDebug);
                    foreach ((string blueprint, int number) in currentItemBlueprints)
                    {
                        Debug.Divider(4, HONLY, Count: 25, Indent: indent, Toggle: doDebug);
                        if (blueprintItemBlueprints.ContainsKey(blueprint))
                        {
                            Debug.Entry(4, $"Before: {blueprint}, {number}", Indent: indent + 1, Toggle: doDebug);
                            blueprintItemBlueprints[blueprint] -= number;
                            if (blueprintItemBlueprints[blueprint] < 1) blueprintItemBlueprints.Remove(blueprint);
                            Debug.Entry(4, $" After: {blueprint}, {number}", Indent: indent + 1, Toggle: doDebug);
                        }
                    }
                    Debug.Divider(4, HONLY, Count: 25, Indent: indent, Toggle: doDebug);
                    ...
                    Debug.Entry(4, $"? if ({nameof(doReequip)})", Indent: indent, Toggle: doDebug);
                    Debug.LoopItem(4, $"{nameof(doReequip)}", $"{doReequip}",
                        Good: doReequip, Indent: ++indent, Toggle: doDebug);
                    if (doReequip)
                    {
                        Debug.Entry(4, $"> foreach ((string blueprint, int number) in blueprintItemBlueprints)", Indent: indent, Toggle: doDebug);
                        foreach ((string blueprint, int number) in blueprintItemBlueprints)
                        {
                            Debug.Divider(4, HONLY, Count: 25, Indent: indent, Toggle: doDebug);
                            for (int i = 0; i < blueprintItemBlueprints.Count; i++)
                            {
                                Debug.CheckYeh(4, $"{blueprint} added to inventory", Indent: indent + 1, Toggle: doDebug);
                                Creature.Inventory.AddObjectToInventory(GameObjectFactory.Factory.CreateObject(blueprint));
                            }
                        }
                        Debug.Divider(4, HONLY, Count: 25, Indent: indent, Toggle: doDebug);
                        Debug.Entry(4, $"x foreach ((string blueprint, int number) in blueprintItemBlueprints) >//", Indent: indent, Toggle: doDebug);
                    }
                    Debug.Entry(4, $"x if (doReequip) ?//", Indent: --indent, Toggle: doDebug);
                }
                else
                {
            ...
            {
                Debug.CheckNah(4, $"Creature null or lacks inventory", Indent: --indent, Toggle: doDebug);
            }
            Debug.Entry(4, $"x if (!Creature.Is(null) and Creature.HasPart<Inventory>()) ?//", Indent: indent, Toggle: doDebug);
