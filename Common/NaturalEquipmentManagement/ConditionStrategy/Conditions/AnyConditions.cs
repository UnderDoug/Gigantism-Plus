        ...
        public override bool NotCheck(T Subject)
        {
            ...
            if (!results.IsNullOrEmpty())
            {
                foreach (bool result in results)
                {
                    if (result)
                    {
                        Debug.CheckNah(4, $"{GetType().Name}.{nameof(NotCheck)}({typeof(T).Name} Subject): {false}", Indent: indent + 2, Toggle: doDebug);
                        Debug.Divider(4, HONLY, 40, TANDR, Indent: indent + 1, Toggle: doDebug);
                        Debug.LastIndent = indent;
                        return false;
                    }
                }
                Debug.CheckYeh(4, $"{GetType().Name}.{nameof(NotCheck)}({typeof(T).Name} Subject): {true}", Indent: indent + 2, Toggle: doDebug);
                Debug.Divider(4, HONLY, 40, TANDR, Indent: indent + 1, Toggle: doDebug);
                Debug.LastIndent = indent;
                return true;
            }
